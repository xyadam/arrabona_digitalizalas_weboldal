const LANG_CHAT_HU = {
    hero: {
        title: "Libra AI",
        subtitle: "Beszélgetés a dokumentumaidhoz - AI-alapú keresés és elemzés"
    },
    intro: {
        title: "Könyvek, Jegyzetek, Dokumentumok - Könnyen Kereshető",
        description: "Feltöltse a bescannelt könyveket, PDF dokumentumokat és jegyzeteket. Mi feldolgozzuk, és egy AI chat interfacen keresztül kérdezhet rájuk. Természetes nyelvű kérdésekkel, pontos forráskövetéssel."
    },
    features: {
        title: "Miért válassza a Libra AI-t?",
        upload: {
            title: "Dokumentumok feltöltése",
            desc: "PDF-ek, Word dokumentumok, bescannelt könyvek - minden formátum támogatott. OCR feldolgozás és AI indexálás."
        },
        chat: {
            title: "AI Chat Interface",
            desc: "Természetes nyelvű kérdésekben tud rákérdezni a dokumentumokra, szűrheti a keresést specifikus könyvekre."
        },
        search: {
            title: "Intelligens Keresés",
            desc: "RAG-alapú keresés: még a legkisebb említés is megtalálható az Ön dokumentumaiban."
        },
        sources: {
            title: "Forráskövetés",
            desc: "Az AI pontosan jelzi, melyik dokumentum melyik részéből jött a válasz - teljes hivatkozások."
        }
    },
    workflow: {
        title: "Hogyan működik?",
        subtitle: "Három egyszerű lépésben az AI adatbázisig",
        step1: {
            title: "Dokumentumok feltöltése és feldolgozása",
            desc: "Digitalizálási munka - feldolgozzuk az Ön dokumentumait. Bescannelt könyv oldalak, PDF-ek, Word fájlok OCR feldolgozása. Az AI szövegként értelmezi és indexeli a tartalmakat.",
            features: [
                "Bescannelt könyv oldalak feldolgozása",
                "PDF dokumentumok támogatása",
                "Word és egyéb szövegfájlok",
                "Automatikus OCR és indexálás"
            ]
        },
        step2: {
            title: "Chat indítása és kérdezés",
            desc: "Beszélgetés az AI-val a feldolgozott dokumentumokról. Szabadon kérdezhet, szűrheti egy vagy több könyvre a keresést. Az online AI provider költsége az ügyfelet terheli (~10 Ft/kérdés).",
            features: [
                "Természetes nyelvű kérdések",
                "Szűrés specifikus könyvekre",
                "Conversation memory",
                "AI költsége: ~10 Ft/kérdés"
            ]
        },
        step3: {
            title: "Keresés és elemzés",
            desc: "Precíz keresés az egész dokumentum adatbázisban. Forráskövetés: melyik oldalakból jött a válasz. Összefoglalások, elemzések, jegyzetfelvétel az AI-val.",
            features: [
                "Teljes szöveg keresés",
                "Forráskövetés és hivatkozások",
                "Dokumentum szűrés",
                "Szöveges összefoglalások"
            ]
        }
    },
    pricingNote: {
        title: "Árazás - Digitalizálási költség",
        intro: "Az alábbi árak a dokumentumok feldolgozásáért (digitalizálás, OCR, AI indexálás) szóló egyszeri díjak.",
        aiCostNote: "Az AI chat használatáért külön fizetni kell az AI provider-nek (tipikusan Gemini, GPT-4, stb.). Egy átlagos kérdés költsége ~10 Ft az ügyfél AI subscription-ja alapján.",
        documentNote: "Az Ön dokumentumait kereshetővé tesszük a feldolgozás során - ezt a díjat itt fizeti meg."
    },
    pricing: {
        title: "Árazás",
        subtitle: "Válassza ki az Önnek megfelelő csomagot",
        package1: {
            name: "Kis csomag",
            range: "1 - 10 000 oldal",
            price: "80",
            unit: "HUF / oldal",
            features: [
                "Dokumentum feldolgozása és OCR",
                "AI adatbázis létrehozása",
                "Chat interface engedélyezése",
                "RAG-alapú keresés",
                "Forráskövetés"
            ]
        },
        package2: {
            name: "Közepes csomag",
            range: "10 000 - 50 000 oldal",
            price: "70",
            unit: "HUF / oldal",
            popular: "Népszerű",
            features: [
                "Dokumentum feldolgozása és OCR",
                "AI adatbázis létrehozása",
                "Chat interface engedélyezése",
                "RAG-alapú keresés",
                "Forráskövetés"
            ]
        },
        package3: {
            name: "Nagy csomag",
            range: "50 000+ oldal",
            price: "60",
            unit: "HUF / oldal",
            features: [
                "Dokumentum feldolgozása és OCR",
                "AI adatbázis létrehozása",
                "Chat interface engedélyezése",
                "RAG-alapú keresés",
                "Forráskövetés"
            ]
        }
    },
    callout: {
        title: "Dokumentumok feldolgozása",
        description: "Sendlje el a dokumentumait, mi feldolgozzuk és kereshetővé tesszük őket. Az online AI chat használata külön költség az Ön AI provider-e alapján."
    },
    contact: {
        title: "Kapcsolat",
        subtitle: "Vegye fel velünk a kapcsolatot további információkért",
        company: "Arrabona",
        note: "Digitalizálási munka - mi feldolgozzuk az Ön dokumentumait. Az AI chat használata külön díj.",
        form: {
            name: "Név",
            email: "Email",
            phone: "Telefon (opcionális)",
            message: "Üzenet (kérjük írja le, milyen dokumentumokkal rendelkezik)",
            submit: "Üzenet küldése"
        }
    },
    footer: {
        rights: "Minden jog fenntartva.",
        backLink: "← Vissza az Arrabona szolgáltatásokhoz"
    }
};
